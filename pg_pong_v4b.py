"""
v4b — 4 parallel envs + pure CPU batch. No GPU at all.
Theoretically fastest for small FC networks.

Usage:
    python pg_pong_v4b.py
    python pg_pong_v4b.py --resume
    python pg_pong_v4b.py --num-envs 8
"""
...
def main():
    ...
    if args.resume:
        model.load_state_dict(torch.load("save_torch_v4b.pt", map_location=DEVICE, weights_only=True))
        print("Loaded checkpoint save_torch_v4b.pt")
    ...
    csv_file = open("train_log_v4b.csv", "w", newline="")
    ...
    while True:
        ...
        # batch inference — all on CPU, no sync needed
        with torch.no_grad():
        ...
        for i in range(num_envs):
            ...
            if episode_number % 100 == 0:
                torch.save(model.state_dict(), "save_torch_v4b.pt")
