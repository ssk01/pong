"""
PyTorch v3 — MPS (Apple GPU) + batch forward/backward.
Same as v2 but runs on GPU, batches kept on device to minimize transfers.

Usage:
    python pg_pong_torch_v3.py
    python pg_pong_torch_v3.py --render
    python pg_pong_torch_v3.py --resume
"""
...
def main():
    ...
    print(f"Train device: {DEVICE}, Inference: CPU")
    model = PongPolicy().to(DEVICE)
    # CPU copy for fast per-step inference (no CPU<->GPU transfer)
    model_cpu = PongPolicy()
    model_cpu.load_state_dict(model.state_dict())

    if args.resume:
        model.load_state_dict(torch.load("save_torch_v3.pt", map_location=DEVICE, weights_only=True))
        model_cpu.load_state_dict(model.state_dict())
        print("Loaded checkpoint save_torch_v3.pt")
    ...
    csv_file = open("train_log_v3.csv", "w", newline="")
    ...
    while True:
        ...
        with torch.no_grad():
            aprob = model_cpu(torch.from_numpy(x)).item()
        ...
        if terminated or truncated:
            ...
            if episode_number % BATCH_SIZE == 0:
                optimizer.step()
                optimizer.zero_grad()
                # sync weights to CPU model — direct tensor copy, faster than load_state_dict
            ...
            if episode_number % 100 == 0:
                torch.save(model.state_dict(), "save_torch_v3.pt")
