...
修改 reward 机制：每次成功回球给 +0.01 小奖励，丢球仍然 -1，得分仍然 +1。让 agent 先学会"稳定回球"再学"打角度得分"。

**核心洞察**：长 rally 中的每一步成功回球都是好动作，不该被最终丢球的 -1 否定。当前系统通过折扣只是"少惩罚"前面的步，而不是奖励它们。

**风险**：回球奖励过大可能导致 agent 只学安全回球不打角度。

### 实验 6: PPO — 同一批样本多次更新

每批样本做 3-10 次梯度更新，用 clip 约束限制策略偏移。样本效率提升 5-10 倍。预期 2000-3000 局到 0 分（vs PG 的 15000 局）。

### 实验 7: Decision Transformer 风格
...
| 5 reward shaping | 可能加速早期 | 密集 reward 信号，但可能改变最优策略 |
| 6 PPO | 最快 | 5-10x 样本效率提升 |
