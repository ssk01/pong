...
| 版本 | 局数 | Running Mean | 耗时 | 结果 |
|------|------|-------------|------|------|
| **v2 batch CPU** | **16221** | **+0.02** | ~170 min | 第一个破 0 |
| **v0 NumPy** | **14319** | **+0.07** | ~175 min | 第二个破 0（局数最少） |
| v5 optimized | 35320 | -1.78→NaN | ~150 min | 梯度爆炸崩溃 |
| v4b multi-env CPU | 8118 | -15.16 | 65 min | 提前终止（省 CPU） |
| **recorder** | **24720** | **+5.09** | ~240 min | 攒够样本，到 +5 停止 |

**关键发现**：
- v0（NumPy）用更少的局数到达 0，因为每局步数更多（学会接球后 rally 变长）
- v5 在接近 0 时梯度爆炸（无 gradient clipping），教训：PG 后期需要梯度裁剪
- PG 在 +3~+5 附近震荡，方差大难以突破，这是 PPO 要解决的问题
- 学得越好每局越长，ep/s 越低 — "成功的代价"

**样本数据**：recorder 共攒了 24720 局样本在 `replay_data/`，覆盖 -21 到 +5 全阶段，足够做所有离线实验。
